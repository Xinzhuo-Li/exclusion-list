...
Detailed operator guide for the six-state Medicaid exclusion list pipeline (MD, MA, MI, MS, MT, NE). For a quick start, see [README.md](../README.md).
...
**Output:** `docs/run_manifest_YYYYMMDD_HHMMSS.json`
...
| Dedup logs (if any dropped) | `docs/dedup_dropped_{state}.json` |
...
**Output:** `docs/validation_report_YYYYMMDD_HHMMSS.json`
...
**Output:** `docs/quality_audit_YYYYMMDD_HHMMSS.json`
...
3. Review new counts in terminal output and `docs/validation_report_*.json`.
4. If counts changed **intentionally**, update baselines in two places:
   ...
   - Record counts table in `docs/DATA_INVENTORY.md`
